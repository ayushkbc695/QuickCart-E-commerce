      ...
      <div>
        {
          cartData.map((item, idx) => {
            ...
            const prouctData = products.find((product) => product._id === item._id );
            return (
              <div key={idx} className='py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
                <div className='flex items-start gap-6'>
                  <img className='w-16 sm:w-20' src={prouctData.image[0]} alt="" />
                  <div>
                    <p className='text-sm sm:text-lg font-normal'>{prouctData.name}</p>
                    <div className='flex items-center gap-5 mt-2'>
                      <p>{currency}{prouctData.price}</p>
                      <p className='px-2 py-1 sm:px-3 border border-gray-200 bg-slate-50'>{item.size}</p>
                    </div>
                  </div>
                </div>
      ...
      <div className='flex justify-end my-20'>
        <div className='w-full sm:w-[450px]'>
          ...
          <div className='w-full text-end'>
            <button onClick={() => navigate('/place-order')} className='bg-black text-white text-sm my-8 px-8 py-3 cursor-pointer rounded'>PROCCED TO CHECKOUT</button>
